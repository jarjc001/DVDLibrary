...
public class DVDLibraryViewConsoleImpl {
    /**Declaration of the UserIO*/
    private UserIO io;

    /**Constructor
     * @param io UserIOConsoleImpl */
    public DVDLibraryViewConsoleImpl(UserIO io) {
        this.io = io;
    }
     ...
     */
    public boolean displayDvdId(Dvd dvd){
        if(dvd == null){
            io.print("Couldn't find DVD in Library");
        }else {
     ...
     * @param dvd The DVD the user wants the info about
     */
    public boolean displayDvdTitle(Dvd dvd){
        if(dvd == null){
            io.print("Couldn't find DVD in Library");
        }else {
            io.print(dvd.toString());
        }
        return repeatAction("search for another DVD from Title");
    }
